...
ENV CGO_ENABLED=0 \
    ...
    GOARCH=amd64
ARG VERSION=dev
WORKDIR /go/src/github.com/johnearl92/xendit-account-service
COPY go.* ./
COPY . .
# RUN go build -mod vendor -o /go/bin/xendit-account-service -ldflags "-X main.version=${VERSION} -w -s"
RUN GO111MODULE=on go build -mod=vendor -o /go/bin/xendit-account-service -ldflags "-X main.version=${VERSION} -w -s"
...
COPY --from=build /go/bin/xendit-account-service /xendit-account-service
...
##
ENTRYPOINT [ "/xendit-account-service" ]
CMD [ "serve" ]