...
#################################################################
DOCKER_REGISTRY=
APP=xendit-account-service
VERSION=latest
TAG=latest
PROJECT=jagbay01
IMAGE=${DOCKER_REGISTRY}${PROJECT}/${APP}:${TAG}

NAMESPACE=xendit-account-service
HELM_CHART=xendit-account-service
VALUES_FILE=helm/xendit-account-service/values/dev.yaml
...
deploy-helm:
	helm upgrade ${HELM_CHART} --install \
		...
		--values ${VALUES_FILE} helm/xendit-account-service
	@echo 'Done.'
